...
import { getWarpcastSharableLink } from "./utils";
...
const handleRequest = frames(async (ctx) => {
  try {
            ...
            <div tw="absolute top-0 left-0 w-full h-full flex flex-col justify-start p-[20px]">
              <div tw="flex flex-col p-[40px] mt-[200px] w-auto text-white">
                ...
                <p tw="text-center text-[42px] mx-auto">
                  (the frame is calculating your mutuals with your potential
                  frens 👀)
                </p>
              </div>
            </div>
    ...
    const userName = ctx.message?.requesterUserData?.username;
    const frensUsernames = frens.map(
      (fren) => fren.profileHandle || fren.profileName || "n.a"
    );
    const sharableWarpcastLink = getWarpcastSharableLink(frensUsernames);
    ...
    return {
      ...
      buttons: [
        ...
        <Button action="link" key="4" target={sharableWarpcastLink}>
          Share
        </Button>,
      ],
      imageOptions: {
    ...
    console.error("Error in loading frame", error);
    return {
